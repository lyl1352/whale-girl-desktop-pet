...
| 路径 | 内容 | 说明 |
|---|---|---|
| `vendor/dsh-whale-girl-pet.client.js` | 浏览器半侧完整代码 | 原样 vendored，仅 **1 处**小改动（见下） |
...
### 对原版 client.js 的全部改动（共 1 处）

**`handleFeed()` 里清掉工作轮播定时器**
```js
...
> 注意：**没有**把 `吃小鱼干` 并进 `WORKING_POOL`。工作轮播池保持原版的 4 项，
> 所以只有「手动投喂」和「充值到账」才会吃零食，工作时不会自己随机掏零食吃。
