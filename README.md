...
| 🎮 **全屏游戏自动退出** | 检测到有程序铺满整个显示器（含无边框全屏）就**退出进程**，把 500MB 内存还给游戏 |
| 🔔 **常驻保障** | 开机自启 + DSH 启动插件 + 可选守护脚本，保证她一直在 |
...
> **第 3 步是可选的。** `koffi` 是一个纯 N-API 的 FFI（Electron 下免重编译），
> 用来直接调 `user32.dll` 判断「前台窗口有没有铺满显示器」。
> 不装也能跑，只是少了全屏自动退出 —— 托盘菜单里那一项会显示为不可用。

### 🎮 全屏游戏自动退出怎么判定

拿**前台窗口的矩形**和**它所在显示器的矩形**比：完全覆盖就算全屏。

| 场景 | 窗口矩形 | 判定 |
|---|---|---|
| 无边框全屏游戏（多数现代游戏） | 0,0 - 3840,2160（= 显示器） | ✅ 全屏 |
| 独占全屏游戏 | 通常还略大一圈（实测 Forza 是 -13,-13 - 3853,2173） | ✅ 全屏 |
| 最大化的普通窗口 | 只覆盖**工作区**，盖不住任务栏 | ❌ 不算 |
| 桌宠自己（避让模式下铺得很宽） | 到不了显示器底边 | ❌ 不算（并且显式排除了自家 HWND） |

判定命中后**直接退出进程**（不是隐藏）—— 她占约 500MB 内存，玩全屏游戏时这笔钱该还回去。

轮询间隔 2 秒，纯 FFI 调用（几微秒，不启进程、不写脚本），
所以不会像「隐藏 PowerShell 守护」那样踩杀软的启发式。

**退出后怎么回来？** 退出前往 `~/.whale-pet-desktop/game-running.json` 写了个标记，
里面是那个游戏进程的 PID：

- **DSH 启动插件**每次要拉她之前先读这个标记：游戏进程还活着就不拉，
  退出后自动清标记并把她拉回来（60 秒内）
- 没有插件时（比如刚装完还没重启过 DSH），双击启动文件夹里的快捷方式即可
- 标记超过 6 小时自动失效（防 PID 复用把她永久锁死）

不想用的话，托盘菜单里把「全屏游戏时自动退出」取消勾选即可。
